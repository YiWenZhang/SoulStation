...
"""
    ...
    """
        ...
        user_task = f"""
    【参考基准分数】
    {base_scores if base_scores else "暂无"}

    【最新对话记录】
    {conversation_text}

    【输出要求】
    返回严格 JSON (diagnosis_summary 和 scores)。

    【评分核心逻辑】
    1. **对比修正**：请对比上述【参考基准分数】，根据对话中患者表现出的症状缓解或加重，给出最新的量化得分。
    2. **动态体现**：严禁无视基准分数直接打分。如果对话显示某维度有好转，得分应在基准分基础上适当下降；反之上升。
